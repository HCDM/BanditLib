...
class NeuralUCBUserStruct:
    def __init__(
        ...
        self.model = extend(
            Network(feature_dim=featureDimension, mlp_dims=self.mlp_dims).to(self.device)
        )
    ...
    def updateParameters(self, gradient, click):
        ...
        self.AInv -= torch.matmul(
            torch.matmul(torch.matmul(self.AInv, gradient.view(-1, 1)), gradient.view(1, -1)),
            self.AInv,
        ) / (
            1
            + torch.matmul(
                torch.matmul(gradient.view(1, -1), self.AInv),
                gradient.view(-1, 1),
            )
        )
    ...
    def update_model(self):
        ...
        optimizer = torch.optim.Adam(
            self.model.parameters(), lr=self.learning_rate, weight_decay=self.lambda_ / num_data
        )
        ...
        for i in range(self.epoch):
            ...
            for j, batch in enumerate(dataloader):
                ...
                # )
                context_feature = (
                    batch["context"].clone().detach().float().requires_grad_(True).to(self.device)
                )
    ...
    def decide(self, pool_articles, userID, k=1):
        ...
        gradient_list = torch.cat(
            [
                ...
                for p in self.users[userID].model.parameters()
            ],
            dim=1,
        )
        # calculate CB
        var_matrix = torch.matmul(
            torch.matmul(gradient_list, self.users[userID].AInv),
