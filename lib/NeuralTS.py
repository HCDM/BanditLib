...
class NeuralTSAlgorithm(BaseAlg):
    def __init__(self, arg_dict, init="zero"):  # n is number of users
        ...
        for i in range(arg_dict["n_users"]):
            self.users.append(
                NeuralTSUserStruct([], arg_dict["dimension"], arg_dict["lambda_"], arg_dict["mlp"], arg_dict["epoch"],
    ...
    def decide(self, pool_articles, userID, k=1):
        ...
        pta_matrix = torch.normal(mean_matrix.view(-1), self.alpha*torch.sqrt(torch.diag(var_matrix)).view(-1))
