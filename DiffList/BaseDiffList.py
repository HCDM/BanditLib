class BaseDiffList():
	...
	def includes(self, alg_name):
		return self.diff_list.has_key(alg_name)

	def initial_write(self, f):
		f.write(',' + ','.join([str(alg_name)+self.name for alg_name in self.diff_list.iterkeys()]))

	def iteration_write(self, f):
		f.write(',' + ','.join([str(self.diff_list[alg_name][-1]) for alg_name in self.diff_list.iterkeys()]))
