...
def createW(gen):
    ...
    if 'clusterfile' in gen:
        ...
        W = initializeW_label(nClusters, rewardManagerDict['relationFileName'], label, args.diagnol,
        ...
        normalizedNewW, newW, label = initializeW_clustering(OriginaluserNum, rewardManagerDict['relationFileName'],
                                                             nClusters)
        rewardManagerDict['label'] = label
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', required=False, choices=['LastFM', 'Delicious'],
                        help='Select dataset to run. No Selection resuts in simulated rewards')
    parser.add_argument('--clusterfile', dest="clusterfile", help="input an clustering label file", metavar="FILE",
    ...
    # if smaller or equal to 0 or larger or enqual to usernum, matrix is fully connected

    # Parameters for GOBLin
    rewardManagerDict['Gepsilon'] = 1

    user['default_file'] = os.path.join(sim_files_folder,
    ...
    if use_coUsers:
        UM = CoUserManager(context_dimension + latent_dimension, user, argv={'l2_limit':    1, 'sparseLevel': n_users,
                                                                             'matrixNoise': rewardManagerDict[
                                                                                                'matrixNoise']})
    else:
    ...
    articlesFilename = os.path.join(sim_files_folder, "articles_" + str(n_articles) + "context_" + str(
    ...
    system_params = {'context_dim': context_dimension, 'latent_dim': latent_dimension, 'n_users': n_users,
                     'n_clusters':  nClusters, 'n_articles': n_articles}
