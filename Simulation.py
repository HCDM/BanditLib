		...
		for iter_ in range(self.testing_iterations):
			...
			for u in self.users:
				...
				for alg_name, alg in algorithms.items():
					...
					if alg.CanEstimateUserPreference:
						ThetaDiff[alg_name] += self.getL2Diff(u.theta[:self.context_dimension], alg.getTheta(u.id))
					if alg.CanEstimateCoUserPreference:
...
if __name__ == '__main__':
	...
	if args.alg == 'factorUCB':
		algorithms['FactorUCB'] = FactorUCBAlgorithm(context_dimension = context_dimension, latent_dimension = latent_dimension, alpha = 0.05, alpha2 = 0.025, lambda_ = lambda_, n = n_users, itemNum=n_articles, W = simExperiment.getW(), init='random', window_size = -1)	
		algorithms['LinUCB'] = N_LinUCBAlgorithm(dimension = context_dimension, alpha = alpha, lambda_ = lambda_, )
	if args.alg == 'CoLin':
		algorithms['CoLin'] = AsyCoLinUCBAlgorithm(dimension=context_dimension, alpha = alpha, lambda_ = lambda_, n = n_users, W = simExperiment.getW())
